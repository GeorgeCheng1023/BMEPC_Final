...
svm_lin_acc=[];svm_lin_sens=[];svm_lin_spec=[];svm_lin_scores=[];svm_lin_labels=[]
svm_rbf_acc=[];svm_rbf_sens=[];svm_rbf_spec=[];svm_rbf_scores=[];svm_rbf_labels=[]
knn_acc=[];knn_sens=[];knn_spec=[];knn_scores=[];knn_labels=[]
tree_acc=[];tree_sens=[];tree_spec=[];tree_scores=[];tree_labels=[];
...
svm_lin_acc = [svm_lin_acc; a1];svm_lin_sens = [svm_lin_sens; s1];svm_lin_spec = [svm_lin_spec; sp1];
svm_lin_scores = [svm_lin_scores; sc1];svm_lin_labels = [svm_lin_labels; lbl1];
[a2, s2, sp2, sc2, lbl2] = train_svm_rbf(Xtr, Ytr, Xte, Yte, 'BoxConstraint', 50, 'KernelScale', 2);
svm_rbf_acc = [svm_rbf_acc; a2];svm_rbf_sens = [svm_rbf_sens; s2];svm_rbf_spec = [svm_rbf_spec; sp2];
svm_rbf_scores = [svm_rbf_scores; sc2];svm_rbf_labels = [svm_rbf_labels; lbl2];
[a3, s3, sp3, sc3, lbl3] = train_knn(Xtr, Ytr, Xte, Yte, 'NumNeighbors', 5);
knn_acc = [knn_acc; a3];knn_sens = [knn_sens; s3];knn_spec = [knn_spec; sp3];
knn_scores = [knn_scores; sc3];knn_labels = [knn_labels; lbl3];
[a4, s4, sp4, sc4, lbl4] = train_decision_tree(Xtr, Ytr, Xte, Yte, 'MinLeafSize', 10, 'MaxNumSplits', 5);
tree_acc = [tree_acc; a4];tree_sens = [tree_sens; s4];tree_spec = [tree_spec; sp4];
tree_scores = [tree_scores; sc4];tree_labels = [tree_labels; lbl4];
end

disp('results:')
names={'SVM (Linear, C=10)';'SVM (RBF, C=50)';'k-NN (k=5)';'Decision Tree (ML=10)'};
